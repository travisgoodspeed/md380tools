...
int     (* const md380_spiflash_read)(void *dst, long adr, long len) = 0x0802fd83;
void    (* const md380_spiflash_write)(void *dst, long adr, long len) =0x0802fe6b;
int     (* const md380_spiflash_security_registers_read)(void *dst, long adr, long len) = 0x080301bd;
void    (* const md380_spiflash_enable)() = 0x0802fe37;
void    (* const md380_spiflash_disable)() = 0x0802fe53;
void    (* const md380_spiflash_wait)()=0x0802fe15;
void    (* const md380_spiflash_block_erase64k)(uint32_t adr)=0x0802fbb7;
void    (* const md380_spiflash_sektor_erase4k)(uint32_t adr)=0x0802fb83;
uint8_t (* const md380_spi_sendrecv)(INT8U data) = 0x0802fdc9; // SPI1




void (* const gfx_drawtext)(wchar_t *str,    //16-bit, little endian.
                            short sx, short sy, //Source coords, maybe?
		            short x, short y,   //X and Y position
		            int maxlen) = 0x0800D88B;
void (* const gfx_drawbmp)(char *bmp,
                           int idx,
                           uint64_t pos) = 0x08022887;

void (* const gfx_drawtext2)(wchar_t *str,    //16-bit, little endian.
                             int x, int y,   //X and Y position, Unit unknown
                             int unknown) = 0x0801cf1d; // max 19 char ???

void (* const gfx_chars_to_display)(wchar_t *str, int x, int y, int unknown) = 0x0801cf1d;

void (* const gfx_select_font)(void *p)=0x8020975;
void (* const gfx_set_bg_color)(int color)=0x801c5e1;
void (* const gfx_set_fg_color)(int color)=0x801c5e9;

void (* const gfx_blockfill)(int xmin, int ymin, int xmax, int ymax)=0x0801cb05;

//! Function that handles the end of a DMR call.
void* (* const dmr_call_end)(void *pkt) = 0x0803f33d;
//! Function that handles a DMR call.
void* (* const dmr_call_start)(void *pkt) = 0x0803ec87;
//! Function that handles a DMR SMS.
void* (* const dmr_handle_data)(void *pkt, int len) = 0x0804b66d;
//! Function that handles an incoming SMS.
void* (* const dmr_sms_arrive)(void *pkt)=0x0803f03d;
//! Function that applies a squelch.
void (* const dmr_apply_squelch)(char *dmr_squelch_firsthing, int dmr_squelch_mode)=0x080303b5;
//! Called before unsquelching.
int (*const dmr_before_squelch)()=0x0803ef6d;
...
int (* const usb_upld_handle)(void*, char*, int, int)=0x0808d3d9;
//! This returns a USB packet to the host from the upload handler.
int (* const usb_send_packet)(void*, char*, uint16_t)=0x080577af;


//Original handler for the DFU DNLD event.
int (* const usb_dnld_handle)()=0x0808ccbf;//2.032
...
INT8U (* const OSTaskCreateExt)(void (*task)(void *pd), void *pdata, OS_STK *ptos, INT8U prio, INT16U id, OS_STK *pbos, INT32U stk_size, void *pext, INT16U opt)=0x804bbf5;
void* (* const OSTaskNameSet)(INT8U prio, INT8U *pname, INT8U *perr)=0x804bcc1;

int (* const OS_ENTER_CRITICAL)()    = 0x08041df9;
void (*const OS_EXIT_CRITICAL)(int) = 0x08041e01;

//! OSSem Create Hook used the place from 2 OSSemCreate Calls
OS_EVENT *(* const OSSemCreate)(uint16_t) = 0x803da2c+1;
OS_EVENT  ** const OSSemCreate_hook0_event_mem=0x2001d124;
OS_EVENT  ** const OSSemCreate_hook1_event_mem=0x2001d128;

void    (* const OSSemPend)(OS_EVENT *pevent, uint32_t timeout,  uint8_t *perr)=0x803da78+1;
uint8_t (* const OSSemPost)(OS_EVENT *pevent)=0x803db68+1;

uint8_t (*const md380_OSMboxPost)(OS_EVENT *pevent, void *pmsg)=0x080303b4+1;
...
int (* const read_channel_switch)()=0x0804d269;




void (* const c5000_spi0_readreg)(int reg, char*buf)=0x0803e2f5;
void (* const c5000_spi0_writereg)(int reg, int val)=0x0803e2a9;


char* (* const aes_cipher)(char *pkt)=0x080356b1;

//! Unknown AMBE2+ thing.
int (* const ambe_encode_thing)(char *a1, int a2, int *a3, int a4,
                                short a5, short a6, short a7, int a8)=0x08050d91;
//! Decodes an AMBE2+ frame into bits.
int (* const ambe_unpack)(int a1, int a2, char length, int a4)=0x08048c9d;

//! Populates the audio buffer.
int (* const ambe_decode_wav)(int *a1, signed int eighty, char *bitbuffer,
...
wchar_t * (* const md380_itow)(wchar_t *, int value)=0x080172ed;
void (* const md380_RTC_GetDate)(uint32_t RTC_Format, RTC_DateTypeDef *RTC_DateStruct)=0x08026461;
void (* const md380_RTC_GetTime)(uint32_t RTC_Format, RTC_TimeTypeDef* RTC_TimeStruct)=0x0802634b;

uint32_t * const md380_dmr_id=0x2001c65c;

//! Functions and Variabes from the "RTC Timer" - task

void (* const md380_f_4137)()=0x080290c8+1;
void (* const md380_f_4520)()=0x08027ae8+1;
void (* const md380_f_4098)()=0x0804c1d0+1;
void (* const md380_f_4102)()=0x0804c2aa+1;
void( * const md380_f_4225)()=0x0801f044+1;
